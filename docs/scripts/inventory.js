...
];function getById(id){return data.find(item=>item.id.toString()===id.toString())}return{getAll:function(){return data},getById:getById,getName:function(id){return getById(id).name},getFilteredItems:function(queryString){return data.filter(item=>item.name.search(new RegExp(queryString,"i"))>-1)}}}));